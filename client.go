...
// DefaultClientOptions read from the environment (EBIRD_API_KEY, PHOEBE_BASE_URL).
// This should be used to initialize new clients.
func DefaultClientOptions() []option.RequestOption {
	defaults := []option.RequestOption{option.WithHTTPClient(defaultHTTPClient()), option.WithEnvironmentProduction()}
	if o, ok := os.LookupEnv("PHOEBE_BASE_URL"); ok {
		defaults = append(defaults, option.WithBaseURL(o))
	}
	if o, ok := os.LookupEnv("EBIRD_API_KEY"); ok {
		defaults = append(defaults, option.WithAPIKey(o))
	}
	if o, ok := os.LookupEnv("PHOEBE_CUSTOM_HEADERS"); ok {
...
// environment (EBIRD_API_KEY, PHOEBE_BASE_URL). The option passed in as arguments
